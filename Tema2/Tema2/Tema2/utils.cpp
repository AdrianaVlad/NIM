...
double binary_to_real(char* b_string, unsigned long long num_bits_per_dimension, double min_x, double max_x)
{
	...
	x_real *= (max_x - min_x);
	x_real += min_x;
	x_real = std::round(x_real * 1e5) / 1e5;
	return x_real;
}

void real_to_binary(double x_real, char* b_string, unsigned long long num_bits_per_dimension, double min_x, double max_x) {
	x_real = (x_real - min_x) / (max_x - min_x) * ((1ULL << num_bits_per_dimension) - 1);
	int x_int = int(x_real);
	for (int j = num_bits_per_dimension - 1; j >= 0; j--) {
		//b_string[j] = (char)(x_int % 2);
		//x_int /= 2;
		b_string[j] = (x_int & 1) ? 1 : 0;
		x_int >>= 1;
	}
}
